(function (global) {
  ...
  ************/
  ...
  function formatEvent(event) {
    ...
    var eventFormatter = formatters[event.event];

    var logged = [];
    function log(template, parameters, level) {
      var msg = ['<' + level + '>', formatString(template, parameters)];
  ...
  global.formatEvent = formatEvent; // expose as a global variable
  ...
  ***************************************************/
  ...
  function formatString(toFormat, replacements) {
  ...
  */
  function jsonGet(dict, property, defValue) {
    if (property in dict) {
  ...
  */
  function jsonPop(dict, property, defValue) {
    var value = jsonGet(dict, property, defValue);
  ...
  var formatters = {
    ...
    ************/
    ...
    'failed': function (log, event) {
      ...
      jsonPop(res, 'invocation', null);

      var item = jsonGet(res, 'item', null);
      var parsed = jsonGet(res, 'parsed', true);
      var moduleMsg = '';
      if (!parsed) {
        moduleMsg = jsonPop(res, 'msg', null);
      }
      var stderr = jsonPop(res, 'stderr', null);
      var stdout = jsonPop(res, 'stdout', null);
      var returnedMsg = jsonPop(res, 'msg', null);
    ...
    'ok': function (log, event) {
      ...
      var item = jsonGet(res, 'item', null);
      jsonPop(res, 'invocation', null);
      var changed = jsonGet(res, 'changed', false);
    ...
    'unreachable': function (log, event) {
      ...
      if (typeof(res) === 'object') {
        item = jsonGet(results, 'item', null);
      }
